...
\begin{frame}{Word2Vec fits SDR}
  ...
  This suggests fitting Word2Vec-like models with SGD-like algorithms instead, as Word2Vec only needs to recompute row-wise normalization constants. 
\end{frame}
...
\end{center}
At the AMPLab, we are interested in adding linear algebra++ algorithms to the Berkeley Data Analytics Stack.
\vskip1em
Our collaborators at NERSC are investigating Spark as a unified platform for data analytics pipelines with diverse scientific inputs.
\end{frame}
...
\vskip1em
  ...
  \vskip1em
  These datasets are tall and skinny, so 1D partitioning makes sense and fits nicely into Spark's RDD formalism.
\end{frame}
...
We use the following algorithm to compute $\mathbf{A}_k$ when $\mathbf{A}$ is tall and skinny:
 \begin{enumerate}
   \item Compute $\mathbf{V}_k$ as the top $k$ eigenvectors of $\mathbf{A}^T \mathbf{A}$ using a Lanczos eigensolver (ARPACK) with multiple rounds of distributed matrix-vector multiplies. 
